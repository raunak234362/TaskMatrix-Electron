  ...
  const getBatchParticipants = (batch) => {
    ...
    return list.map(r => {
      if (typeof r === 'object' && r !== null) {
        ...
        const name = r.raisedByName || (userObj?.firstName ? `${userObj.firstName} ${userObj.lastName || ''}`.trim() : 'Participant')
        return {
          name,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-gray-100 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">View and manage training batches assigned to you</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          ...
          <p className="text-gray-400 text-sm">You are not currently set as a trainer or participant in any batches.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="px-5 py-4 space-y-2.5 text-sm text-gray-600">
                  ...
                  {batch.createdAt && (
                    <p className="text-[11px] text-gray-400 font-medium">Created {fmt(batch.createdAt)}</p>
                  )}
                </div>
