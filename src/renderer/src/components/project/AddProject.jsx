...
const AddProject = () => {
  ...
  const options = {
    rfqs: rfqData.map((r) => ({ label: `${r.projectName} • ${r.fabricator?.fabName}`, value: String(r.id) })),
    fabricators: fabricators.map((f) => ({ label: f.fabName, value: String(f.id) })),
    departments: departmentDatas.map((d) => ({ label: d.name, value: String(d.id) })),
    managers: users.map((u) => ({ label: `${u.firstName} ${u.lastName}`, value: String(u.id) })),
    teams: teamDatas.map((t) => ({ label: t.name, value: String(t.id) })),
    connectionDesigners: connectionDesigners.map((c) => ({ label: c.connectionDesignerName || c.name, value: String(c.id) })),
    tools: [
