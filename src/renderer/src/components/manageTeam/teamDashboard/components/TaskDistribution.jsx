...
const TaskDistribution = ({ teamStats }) => {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-none border border-black shadow-soft h-full flex flex-col">
      <h3 className="text-2xl font-semibold text-black mb-8 flex items-center gap-3 uppercase tracking-tight">
        <Layers className="text-black" size={24} strokeWidth={2.5} />
        Task Distribution
        <span className="text-[10px] font-bold text-[#4b8a2e] ml-auto bg-green-50 px-3 py-1.5 rounded-none border border-[#6bbd45]/20 uppercase tracking-widest">
          Task Counts
        </span>
      </h3>
            ...
            >
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-black font-bold text-sm tracking-tight transition-colors">
                    {item.label}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-black font-bold bg-white border border-black px-5 py-2 rounded-none text-sm shadow-sm group-hover:border-black transition-all">
                    {item.value || 0}
                  </span>
                  {item.tasks.length > 0 && (
                    <ChevronDown
                      size={18}
                      className={`text-black transition-transform duration-300 ${expandedIndex === index ? "rotate-180" : ""}`}
                    />
                  )}
                </div>
              </div>
                    ...
                    {item.tasks.map((task, tIdx) => (
                      ...
                      >
                        <div className="flex justify-between items-start gap-4 mb-2">
                          <h4 className="text-xs font-bold text-black leading-tight line-clamp-2">
                            {task.name || task.title || "Untitled Task"}
                          </h4>
                          <span
                            className={`px-2 py-1 text-[9px] font-bold uppercase tracking-widest rounded-none shrink-0 border ${task.status === "COMPLETE"
