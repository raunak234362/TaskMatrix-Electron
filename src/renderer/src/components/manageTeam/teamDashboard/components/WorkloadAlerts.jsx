import { UserMinus, Clock, AlertTriangle } from "lucide-react";
        ...
        <div className="p-8 max-h-[250px] overflow-y-auto custom-scrollbar">
          {notAssigned.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {notAssigned.map(m => (
                <div key={m.id} className="px-4 py-2 bg-gray-50 border border-black/5 rounded-xl text-xs font-bold text-black shadow-sm">
                  {m.name}
                </div>
              ))}
            </div>
        ...
        <div className="p-8 max-h-[250px] overflow-y-auto custom-scrollbar">
          {underAssigned.length > 0 ? (
            <div className="grid grid-cols-1 gap-3">
              {underAssigned.map(m => (
                <div key={m.id} className="flex items-center justify-between px-4 py-3 bg-gray-50 border border-black/5 rounded-xl shadow-sm">
                  <span className="text-xs font-bold text-black">{m.name}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black text-black/40 uppercase tracking-widest">{m.assignedHours} hrs</span>
                    <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden border border-black/5">
                      <div className="bg-yellow-500 h-full" style={{ width: `${(Number(m.assignedHours) / 8) * 100}%` }}></div>
                    </div>
                  </div>
                </div>
              ))}
