...
const AllSubmittals = ({ submittalData, projectId }) => {
  ...
  const generalSubmittals = submittals.filter(item => item.isConnectionDesign !== true && String(item.isConnectionDesign).toLowerCase() !== "true");
  const connectionDesignerSubmittals = submittals.filter(item => item.isConnectionDesign === true || String(item.isConnectionDesign).toLowerCase() === "true");

  const displayedSubmittals = activeTab === "CONNECTION_DESIGNER" ? connectionDesignerSubmittals : generalSubmittals;

  const finalSubmittals = displayedSubmittals.filter(item => 
